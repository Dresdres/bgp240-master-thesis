...
public class StockService : IStockService
{
    ...
    public StockService(IStockRepository stockRepository, IOptions<StockConfig> config, ILogger<StockService> logger)
    {
    ...
    public async Task ProcessProductUpdate(ProductUpdated productUpdated)
    {
        using (var txCtx = this.stockRepository.BeginTransaction())
        {
            ...
            {
                await this.stockRepository.RawSQL($"SELECT stock_add_product_transaction_mark('{streamUpdateId}','{productUpdated.version}','{TransactionType.UPDATE_PRODUCT}','{productUpdated.seller_id}','{MarkStatus.SUCCESS}','stock');");
    ...
    {
        await this.stockRepository.RawSQL($"SELECT stock_add_product_transaction_mark('{streamUpdateId}','{productUpdate.version}','{TransactionType.UPDATE_PRODUCT}','{productUpdate.seller_id}','{MarkStatus.ABORT}','stock');");
    ...
    {   
        ...
        using (var txCtx = this.stockRepository.BeginTransaction())
        {
            ...
            if (!items.Any())
            {
                ...
                await this.stockRepository.RawSQL($"SELECT stock_add_checkout_transaction_mark('{streamUpdateId}','{checkout.instanceId}','{TransactionType.CUSTOMER_SESSION}','{checkout.customerCheckout.CustomerId}','{MarkStatus.ERROR}', 'stock');");
            ...
            if (this.config.Streaming)
            {
                if (cartItemsReserved.Count() > 0)
                {
                    ...
                    StockConfirmed stockConfirmed = new StockConfirmed(
                        ...
                        cartItemsReserved,
                        checkout.instanceId);
                    var updatejson = JsonSerializer.Serialize(stockConfirmed);
                    string sql = $"SELECT pg_notify('stock_confirmed', @updateJson)";
                    await this.stockRepository.RawSQLMsg(sql, new NpgsqlParameter("@updateJson", updatejson));
                ...
                if (unavailableItems.Count() > 0)
                {
                    ...
                    if(this.config.RaiseStockFailed){
                        ...
                        var updatejson = JsonSerializer.Serialize(reserveFailed);
                        string sql = $"SELECT pg_notify('stock_failed', @updateJson)";
                        await this.stockRepository.RawSQLMsg(sql, new NpgsqlParameter("@updateJson", updatejson));
                    ...
                    if (cartItemsReserved.Count() == 0)
                    {
                        ...
                        await this.stockRepository.RawSQL($"SELECT stock_add_checkout_transaction_mark('{streamUpdateId}','{checkout.instanceId}','{TransactionType.CUSTOMER_SESSION}','{checkout.customerCheckout.CustomerId}','{MarkStatus.NOT_ACCEPTED}','stock');");
    ...
    {
        await this.stockRepository.RawSQL($"SELECT stock_add_checkout_transaction_mark('{streamReserveId}','{reserveStock.instanceId}','{TransactionType.CUSTOMER_SESSION}','{reserveStock.customerCheckout.CustomerId}','{MarkStatus.ABORT}','stock');");
    ...
    public async Task IncreaseStock(IncreaseStock increaseStock)
    {
        using (var txCtx = this.stockRepository.BeginTransaction())
        {
            ...
            if (this.config.Streaming)
            {
                ...
                var updatejson = JsonSerializer.Serialize(stockItem);
                string sql = $"SELECT pg_notify('stock_update', @updateJson)";
                //await this.stockRepository.RawSQLMsg(sql, new NpgsqlParameter("@updateJson", updatejson));
